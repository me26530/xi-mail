...
const dbInit = {
	...
	async v3_0DB(c) {
		const ADD_COLUMN_SQL_LIST = [
			`ALTER TABLE setting ADD COLUMN auto_ban_months INTEGER NOT NULL DEFAULT 0;`,
			`ALTER TABLE setting ADD COLUMN ban_message TEXT NOT NULL DEFAULT '该邮箱已被封禁';`,
			`ALTER TABLE setting ADD COLUMN random_prefix_length INTEGER NOT NULL DEFAULT 8;`,
			`ALTER TABLE setting ADD COLUMN domain_mapping TEXT NOT NULL DEFAULT '{}';`,
			`ALTER TABLE role ADD COLUMN level INTEGER NOT NULL DEFAULT 100;`
		];
	...
	async v3_1DB(c) {
		const ADD_COLUMN_SQL_LIST = [
			`ALTER TABLE setting ADD COLUMN email_keyword_blacklist TEXT NOT NULL DEFAULT '';`
		];
	...
	async v3_2DB(c) {
		// Add display_id to user table
		try {
			await c.env.db.prepare(`ALTER TABLE user ADD COLUMN display_id INTEGER NOT NULL DEFAULT 0;`).run();
		} catch (e) {}

		// Populate display_id for existing users who have display_id = 0
		try {
			const { results } = await c.env.db.prepare(`SELECT user_id FROM user WHERE display_id = 0`).all();
			for (const row of results) {
				const displayId = Math.floor(Math.random() * 900000000) + 100000000;
				await c.env.db.prepare(`UPDATE user SET display_id = ? WHERE user_id = ?`)
					.bind(displayId, row.user_id).run();
			}
		} catch (e) {
			console.warn('display_id populate:', e.message);
		}
	...
	async v3_3DB(c) {
		// Regenerate all display_ids as alphanumeric format (xxxx-xxxx-xxxx)
		...
		try {
			const { results } = await c.env.db.prepare(`SELECT user_id, display_id FROM user`).all();
			for (const row of results) {
				// Regenerate if empty, 0, or still looks numeric (old format)
				if (!row.display_id || row.display_id === '0' || /^\d+$/.test(String(row.display_id))) {
					await c.env.db.prepare(`UPDATE user SET display_id = ? WHERE user_id = ?`)
						.bind(genId(), row.user_id).run();
				}
			}
		} catch (e) {
	...
	async v3_5DB(c) {
		try {
			await c.env.db.prepare(`ALTER TABLE setting ADD COLUMN reg_key_hint TEXT NOT NULL DEFAULT '';`).run();
		} catch (e) {}
		try {
	...
	async v1_6DB(c) {
		...
		const noticeContent = 'Xi-Mail 临时邮箱服务 🎉\n' +
			...
			'<br>\n' +
			'本服务仅供学习交流，请遵守当地法规，禁止用于违法用途。</span>'
